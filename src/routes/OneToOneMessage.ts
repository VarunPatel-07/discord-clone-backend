...
routes.get(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const Fetch_All_Conversation =
        await database.oneToOneConversation.findMany({
          where: {
  ...
  async (req: any, res: any) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        message:
          "Internal server error while fetching all the conversation messages",
      });
    }
  }
);
...
routes.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      const receiver_id =
        FindConversation.SenderId === req.user_id
          ? FindConversation.ReceiverId
          : FindConversation.SenderId;
