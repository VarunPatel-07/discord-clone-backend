...
import { Server__Image__Uploader, Upload_Image_In_Compressed_Format } from "../../middleware/MulterImageUploader";
...
import { DeleteSpecificDataInRedis, StoreDataInRedis } from "../Helper/StorDataInRedis";
...
routes.post(
  ...
  async (req: any, res: any) => {
    try {
      ...
      if (!imageArr) {
        return res.status(400).json({ message: "the server image is required", success: false });
      }
      ...
      const ServerImageBs64 = Buffer.from(imageArr.serverImage[0].buffer).toString("base64");

      const CloudServerImage: any = await Upload_Image_In_Compressed_Format(ServerImageBs64, 96, 96);
...
routes.get("/get-servers", CheckAuthToken, async (req: any, res: any) => {
  try {
    ...
    if (cacheServerInfo) {
      ...
      return res.status(200).json({ server_info: JSON.parse(cacheServerInfo), success: true });
    }
    ...
    if (!server_info) {
      return res.status(400).json({ message: "No server found", success: false });
    }
...
//
routes.get("/serverInfo/:serverId", CheckAuthToken, async (req: any, res: any) => {
  try {
    ...
    if (cacheServerInfo) {
      // If cached data exists, parse it and return
      return res.status(200).json({ Server__Info: JSON.parse(cacheServerInfo), success: true });
    }
...
//
routes.put("/regenerateInviteCode", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    ...
    if (!serverId) {
      return res.status(400).json({ message: "serverId is required", success: false });
    }
...
//
routes.put("/joinServerWithInviteCode", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    ...
    DeleteSpecificDataInRedis(multiple_server_info);
    if (!inviteCode) {
      return res.status(400).json({ message: "inviteCode is required", success: false });
    }
    ...
    if (!Find_Server) {
      return res.status(400).json({ message: "Server not found", success: false });
    }
...
//? CHANGE MEMBER ROLE
//
routes.put("/changeMemberRole/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
...
//
routes.put("/kickOutMemberFromServer/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
...
//? CREATE NEW CHANNEL
//
routes.put("/createNewChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  ...
  try {
    ...
    if (server_info?.members.some((member: any) => member.userId === req.user_id)) {
      if (
        server_info?.members.some((member: any) => member.role === MemberRole.ADMIN) ||
        server_info?.members.some((member: any) => member.role === MemberRole.MODERATOR)
      ) {
        ...
        if (ChannelType == "TEXT") {
          ...
          StoreDataInRedis(
            cache_channel_key,
            updated_info.channels.filter((channel: any) => channel.type === ChannelType)
          );
        }
        if (ChannelType == "AUDIO") {
          const cache_channel_key = `audio_channel_${serverId}`;
...
//? UPDATE CHANNEL
//
routes.put("/updateChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
...
//? DELETE CHANNEL
//
routes.delete("/deleteChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
    ...
    const channelExists = server.channels.some((channel) => channel.id === channelId);
    if (!channelExists) {
      return res.status(404).json({
        success: false,
...
//? LEAVE SERVER
//
routes.put("/LeaveServer/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
    const multiple_server_info = `multiple_server_info_${req.user_id}`;
    const cache_server_key = `single_server_${serverId}`;
    ...
    StoreDataInRedis(cache_server_key, updated_info);
    res.status(200).json({ message: "Member Left successfully", success: true });
  } catch (error) {
    // // console.log(error);
    return res.status(500).json({
      success: false,
...
//? DELETE SERVER
//
routes.delete("/deleteServer/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
    const multiple_server_info = `multiple_server_info_${req.user_id}`;
    const cache_server_key = `single_server_${serverId}`;
...
//
routes.get("/FetchTextChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
...
//
routes.get("/FetchAudioChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
...
//
routes.get("/FetchVideoChannel/:serverId", CheckAuthToken, multer().none(), async (req: any, res: any) => {
  try {
    const { serverId } = req.params as { serverId: string };
